...
public class Main {
    ...
    void go() {
        ...
        for (int i = 1; i < n; i++) {
            if (i == 1) {
                int add = (a[i] + a[i - 1]) % 10;
                int multiply = (a[i] * a[i - 1]) % 10;
                dp[0][add] += 1;
                dp[0][multiply] += 1;
                set.add(add);
                set.add(multiply);
                ...
                for (int j = 0; j < 10; j++) {
                    if (set.contains(j)) {
                        int add = (j + a[i]) % 10;
                        int multiply = (j * a[i]) % 10;
                        dp[i - 1][add] = (dp[i - 1][add] + dp[i - 2][j]) % mod;
                        dp[i - 1][multiply] = (dp[i - 1][multiply] + dp[i - 2][j]) % mod;
                        list.add(add);
                        list.add(multiply);
                    }
                }
