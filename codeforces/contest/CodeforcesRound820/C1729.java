package contest.CodeforcesRound820;
...
//        out.println(pos.toString());
        int first = s[0] - 'a', last = s[n - 1] - 'a';
        int cost = Math.abs(first - last);
        int m = 0;
        TreeSet<Character> set = new TreeSet<>();
        for (int i = 1; i < n - 1; i++) {
            int value = s[i] - 'a';
            if (value >= Math.min(last, first) && value <= Math.max(first, last)) {
                m++;
                set.add(s[i]);
            }
        }
