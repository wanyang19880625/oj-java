package contest.CodeforcesRound747;
...
public class B1594 {
    ...
    void go() {
        ...
        if (k == 1) {
            ...
            long[] binary = new long[30];
            binary[0] = 1;
            for (int i = 1; i < binary.length; i++) {
                binary[i] = binary[i - 1] * n % mod;
            }
            char[] res = Long.toBinaryString(k).toCharArray();
            long ans = 0;
            for (int i = res.length - 1; i >= 0; i--) {
                if (res[i] == '1') {
                    ans += binary[res.length - 1 - i];
                    ans = ans % mod;
                }
            }
            out.println(ans);
