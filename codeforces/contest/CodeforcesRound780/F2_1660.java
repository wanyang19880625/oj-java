package contest.CodeforcesRound780;
    ...
    void go() {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (map.containsKey(prefix[i])) {
                pos = map.get(prefix[i]);
            }
            pos.add(i + 1);
            map.put(prefix[i], pos);
        }
//        out.println(map.toString());

        long ans = 0;
//        3 -3 0 3 0 -3 -6
//        [1 4]3 [3 5]0 [2 6]-3 [7]-6
        for (int i = n + 1, j = 0; j < 3; j++, i--) {
            int key = i;
            List<Integer> pos = new ArrayList<>();
            while (key >= -n) {
                if (map.containsKey(key)) {
                    pos.addAll(map.get(key));
                }
                key -= 3;
            }
