...
public class E1703 {
    ...
    void go() {
        ...
        for (int i = 0, j = 0; i < n / 2; i++, j++) {
            int s = i, t = j;
            List<Character> up = new ArrayList<>(), right = new ArrayList<>(),
                    down = new ArrayList<>(), left = new ArrayList<>();
            while (t < m) {
                up.add(grid[s][t]);
                t++;
            }
            while (s < m) {
                right.add(grid[s][t - 1]);
                s++;
            }
            while (t > i) {
                down.add(grid[s - 1][t - 1]);
                t--;
            }
            while (s > i) {
                left.add(grid[s - 1][t]);
            ...
            for (int x = 1; x < up.size() - 1; x++) {
                int cnt = (up.get(x) + down.get(x) + left.get(x) + right.get(x))
                        - 4 * '0';
                ans += Math.min(cnt, 4 - cnt);
            }
//            out.println(up, down, right, left);
            int cnt = (up.get(0) + down.get(0) +
                    up.get(up.size() - 1) + down.get(down.size() - 1)) - 4 * '0';
            ans += Math.min(cnt, 4 - cnt);
            m -= 1;
        }
        out.println(ans);
