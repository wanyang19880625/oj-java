package contest.CodeforcesRound589;
 ...
 * @date: 2021/9/2 17:49
 * @url: https://codeforc.es/contest/1228/problem/C
 ***********************/
public class C1228 {
    InputStream is;
    FastWriter  out;
    ...
    void go() {
        long x = nl(), n = nl();
        Map<Long, Integer> xMap = getPrimeFactor(x);
        long mod = (long) 1e9 + 7;
        long ans = 1;
        for (long p : xMap.keySet()) {
            long cnt = 0;
            long tmp = p;
            // 求符合p^k<=n的数量cnt
            while (tmp <= n) {
                cnt += n / tmp;
                // 注意溢出
                if (tmp > Long.MAX_VALUE / p) {
                    break;
                }
                tmp *= p;
            }

            ans = ans * quickpower(p, cnt, mod);
            ans = ans % mod;
        }
        out.println(ans);
    }
     ...
     * @return
     */
    long quickpower(long a, long count, long mod) {
        long result = 1;
        while (count != 0) {
            if ((count & 1) == 1) {
                result = (result * a) % mod;
