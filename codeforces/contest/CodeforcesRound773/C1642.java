package contest.CodeforcesRound773;
...
//        out.println(map.toString());
        ...
        for (long s : set) {
            int count = map.get(s);
            if (count > 0) {
                if (map.containsKey(x * s) && map.get(x * s) > 0) {
                    int value = map.get(x * s);
                    map.put(x * s, value - Math.min(count,value));
                    map.put(s, map.get(s) - Math.min(count,value));
                }
            }
        }
//        out.println(map.toString());
