package contest.CodeforcesRound630;
 ...
 * @date: 2021/2/5 16:26
 * @url: https://codeforc.es/problemset/problem/1332/C
 ***********************/
...
public class C {
    ...
    void go() {
        ...
        for (int i = 0; i < (k + 1) / 2; i++) {
            ...
            for (int j = 0; j < n / k; j++) {
                int v1 = s.charAt(j * k + i) - 'a';
                count[v1]++;
                if (!(k % 2 == 1 && i == k / 2)) {
                    int v2 = s.charAt((j + 1) * k - i - 1) - 'a';
                    count[v2]++;
                }
            }
            int min = total;
            for (int j = 0; j < 26; j++) {
                min = Math.min(total - count[j], min);
            }
            ans += min;
        }
        out.println(ans);
