...
public class D1560 {
    ...
    void go() {
        ...
        for (long x : list) {
            long tmp = x;
            Deque<Long> digit = new LinkedList<>();
            long m = n;
            while (m > 0) {
                digit.addFirst(m % 10);
                m /= 10;
            }
            ans = Math.min(ans, digit.size() + 1);
            Deque<Long> power = new LinkedList<>();
            while (tmp > 0) {
                power.addFirst(tmp % 10);
                tmp /= 10;
            }
            int dsize = digit.size(), psize = power.size();
            int cnt = 0;
            int i = 0, j = 0;
            while (digit.size() > 0 && power.size() > 0) {
                long d = digit.pollFirst();
                long p = power.peekFirst();
                if (d == p) {
                    j++;
                    power.pollFirst();
                } else {
