package contest.CodeforcesRound742;
...
public class D1567 {
    ...
    void go() {
        ...
        int[] carry = new int[t.length()];
        for (int i = 0; i < t.length(); i++) {
            digit[i] = t.charAt(i) - '0';
            carry[i] = (int) Math.pow(10, t.length() - i - 1);
        }
        while (true) {
            int sum = 0;
        ...
        for (int i = 0; i < digit.length; i++) {
            if (digit[i] > 0) {
                for (int j = 0; j < Math.min(n, digit[i]); j++) {
                    list.add(carry[i]);
                    tmp += carry[i];
                }
                if (digit[i] >= n) {
