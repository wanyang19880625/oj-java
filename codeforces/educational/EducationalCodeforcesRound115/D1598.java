package educational.EducationalCodeforcesRound115;
...
/***********************
 ...
 @url: https://codeforc.es/contest/1598/problem/D
 ***********************/
public class D1598 {
    ...
    void go() {
        ...
        for (Map.Entry<Integer,Set<Integer>> entry : aMap.entrySet()) {
             ...
             * 分为三种情况分析:
             * 1.若某个特定a[i](或b[i])的数量为1时,则不论怎么选都是符合条件
             * 2.若a[i](或b[i])的数量为3时,则由题意可知,某个特定的a[i](或b[i])下所有的值均不相同,
             *   也可以得到:任意选三个数据且a[i](或b[i])相同时,无论怎么选都是符合条件
             * 所以,推出只需要计算出a[i](或b[i])的数量都等于2时的重复集合的数量即可。
             * 记录a[i]的数量为count;
