package educational.EducationalCodeforcesRound113;
 ...
 * @date: 2021/9/9 14:14
 * @url: https://codeforc.es/contest/1569/problem/C
 ***********************/
        ...
        if (a[n - 1].equals(a[n - 2])) {
            ...
            if (a[n - 1] > a[n - 2] + 1) {
                ...
                for (int i = count; i <= n - 1; i++) {
                    tmp += fact[i] * infact[i - count] % mod * infact[count] % mod;
                    tmp = tmp % mod;
                }
                out.println((total - tmp * fact[count] % mod * fact[n - 1 - count] % mod + mod) % mod);
            }
        }
    }
